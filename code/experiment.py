...
import numpy as np
from samplers import (run_ps_rwm_once, run_ps_hmc_once, run_ps_ula_once, run_ps_mala_once, run_ps_nuts_once,)


SCALAR_SUMMARY_KEYS = [ "logZ", "final_ess", "acceptance_rate_mean", "acceptance_rate_last",
                        "n_iter", "runtime_sec", "gradient_eval_count", "resampling_steps", ]

ARRAY_SUMMARY_KEYS = ["posterior_mean", "posterior_second_moment",]
...
def _kernel_defaults(kernel_name: str) -> dict[str, Any]:
    ...
    defaults = {"rwm": {}, "hmc": {}, "ula": {}, "mala": {}, "nuts": {},}
...
def _sampler_dispatch(kernel_name: str): 
    ...
    dispatch = {"rwm": run_ps_rwm_once, "hmc": run_ps_hmc_once, "ula": run_ps_ula_once, "mala": run_ps_mala_once, "nuts": run_ps_nuts_once, }
    if kernel_name not in dispatch:
        raise ValueError(f"Unknown kernel '{kernel_name}'")
    return dispatch[kernel_name] 

def _raw_output_path(*, results_root: Path, target_name: str, dimension: int, num_particles: int,
                     num_mcmc_steps: int, kernel_name: str, seed: int) -> Path:
    # output path for the raw data 
    return (results_root / "raw" / target_name / f"D_{dimension}" / f"N_{num_particles}" / f"MCMC_{num_mcmc_steps}" / kernel_name / f"seed_{seed}.json" )

def _summary_output_path(*, results_root: Path, target_name: str, dimension: int, num_particles: int,
                         num_mcmc_steps: int, kernel_name: str) -> Path:
    # output path for the summary of each kernel
    return (results_root / "summary"  / target_name / f"D_{dimension}"  / f"N_{num_particles}" / f"MCMC_{num_mcmc_steps}" / f"{kernel_name}_summary.json")
...
def _run_experiment_worker(args: dict[str, Any]) -> dict[str, Any]:
    ...
    sampler_kwargs = {  "dimension": args["dimension"],
...
def _aggregate_array(arrays: list[np.ndarray]) -> dict[str, Any]:
    ...
    return {"mean": np.mean(stacked, axis=0), "std": std,}

def _summarize_runs(*, config: ExperimentConfig, kernel_name: str, dimension: int,
                    num_particles: int, num_mcmc_steps: int, raw_paths: list[Path]) -> dict[str, Any]:
    # creates summary files from raw files 
    runs = [_load_json(path) for path in raw_paths]

    summary: dict[str, Any] = { "target_name": config.target_name,
...
def _print_config_header(*, kernel_name: str, dimension: int, num_particles: int,
                         num_mcmc_steps: int, num_missing: int, total: int) -> None:
    # prints progress status in kernel so that I now things are working
    print(  f"[kernel={kernel_name} | D={dimension} | N={num_particles} | MCMC={num_mcmc_steps}] "
            f"missing_runs={num_missing}/{total}", flush=True)

def run_experiment_grid(config: ExperimentConfig,
                        results_root: str | Path | None = None) -> list[dict[str, Any]]:
        ...
        for num_particles in config.num_particles_grid:
            for num_mcmc_steps in config.num_mcmc_steps_grid:
                for kernel_name in config.kernels:
                    ...
                    raw_paths = [
                        _raw_output_path(results_root=results_root,
                    ...
                    for seed, raw_path in zip(config.seeds, raw_paths):
                        ...
                        missing_jobs.append({"kernel_name": kernel_name,
                                            "dimension": int(dimension),
                                            "num_particles": int(num_particles),
                                            "num_mcmc_steps": int(num_mcmc_steps),
                                            "seed": int(seed),
                    ...
                    if config.verbose: # printing configuration header
                        _print_config_header(kernel_name=kernel_name,
                                            dimension=dimension,
                                            num_particles=num_particles,
                                            num_mcmc_steps=num_mcmc_steps,
                            ...
                            with ProcessPoolExecutor(max_workers=workers, mp_context=ctx) as executor:
                                ...
                                for future in as_completed(futures): #collect completed jobs
                                    ...
                                    if config.verbose:
                                        ...
                                        print(  f"Run {completed_jobs}/{total_jobs} completed "
                    ...
                    if len(existing_raw_paths) == 0:
                        continue
                    # from the raw files calculate make th summary
                    summary = _summarize_runs(  config=config,
                                                kernel_name=kernel_name,
                                                dimension=dimension,
                                                num_particles=num_particles,
                                                num_mcmc_steps=num_mcmc_steps,
                    ...
                    summary_path = _summary_output_path(results_root=results_root,
                                                        target_name=config.target_name,
                                                        dimension=dimension,
                                                        num_particles=num_particles,
                                                        num_mcmc_steps=num_mcmc_steps,
...
if __name__ == "__main__":
    config = ExperimentConfig(  target_name="gaussian_mixture",
                                algorithm_name="ps",
                                dimensions=[10],

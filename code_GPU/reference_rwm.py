# reference_rwm.py
# CPU reference implementation for SMC + RWM with Robbins-Monro diminishing adaptation
...
def debug_worker_backend() -> dict[str, Any]:
    # Which device JAX is using and which devices are available.
    return {
...
def run_reference_sampler_once( dimension: int,
    ...
    if kernel_name != "rwm":
        raise ValueError(
            f"reference_rwm.py only supports kernel_name='rwm'. "
            f"Got kernel_name={kernel_name!r}. Use the GPU MALA reference for MALA runs."
        )
